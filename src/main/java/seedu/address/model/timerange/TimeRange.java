...
public class TimeRange {
    public static final String MESSAGE_CONSTRAINTS = "TimeRange can take any string"
            + " in a `HHmm-HHmm MINUTES` format where 'HH' stands for hour, 'mm' stands for minute"
            + " and 'MINUTES' stands for duration and is to be replaced by integer.\n"
            + " The start time must be before the end time and both the start time and end time are on the same day.\n"
            + "Example: '1000-1300 120'";
    ...
    public static boolean isValidTimeRangeFormat(String dateTimeRange) {
        ...
        // The duration is guaranteed to be correct as it passes the regex test which checks if it is an integer.
    ...
    private static boolean isValidTimeString(String time) {
        assert time.matches(VALIDATION_TIME_REGEX);
